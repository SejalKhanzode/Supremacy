...
import { getAllAlgorithms } from "../../../services/operations/topicAPI";
import { Outlet,  } from "react-router-dom";

const Algorithms = () => {
  const [algoTopics, setAlgoTopics] = useState([]);
  const [selectedTopic, setSelectedTopic] = useState(null);

  useEffect(() => {
    const fetchAlgoTopics = async () => {
      const result = await getAllAlgorithms();
      setAlgoTopics(result);
    };
    fetchAlgoTopics();
  }, []);
